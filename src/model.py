...
class PlantModel:
    ...
    def AM_bidding_obj_function_seb_jay(self, N_TH, spot_prices, B_volumes, B_prices, balancing_market: BalancingMarket, MTU_start = None):
        ...
        for k in range(0, N_TH):
            L += \
                  - bid_volumes_down[k] * balancing_market.demand_prob_down(spot_prices[k]) * (expected_prices_down[:,k]*(1-gaussian_CDF(bid_prices_down[k], expected_prices_down[:,k], expected_price_sd_down)) + expected_price_sd_down*gaussian_PDF(bid_prices_down[k], expected_prices_down[:,k], expected_price_sd_down)) \
                  - bid_volumes_up[k] * balancing_market.demand_prob_up(spot_prices[k]) * (expected_prices_up[:,k]*(1-gaussian_CDF(bid_prices_up[k], expected_prices_up[:,k], expected_price_sd_up)) + expected_price_sd_up*gaussian_PDF(bid_prices_up[k], expected_prices_up[:,k], expected_price_sd_up))\
                  + bid_volumes_down[k] * spot_prices[k] * balancing_market.activation_prob_down(bid_prices_down[k], spot_prices[k], expected_prices_down[:,k], expected_price_variance_down) \
                  + bid_volumes_up[k] * spot_prices[k] * balancing_market.activation_prob_up(bid_prices_up[k], spot_prices[k], expected_prices_up[:,k], expected_price_variance_up) 

                    # Expected value of payment from TSO to BSP (that's us) for delivering a down-activation
                    # Expected value of payment from TSO to BSP (that's us) for delivering an up-activation
                    # Payment to BRP for deviating from baseline during down-act
                    # Payment to BRP for deviating from baseline during up-act

        L = L/4

        return L

    def AM_baseline_deviation_obj_function(self, N_TH, spot_prices, B_volumes, B_prices, balancing_market: BalancingMarket, MTU_start = None):
        ...
        for k in range(0, N_TH):
            L += bid_volumes_down[k] * spot_prices[k] * balancing_market.activation_prob_down(bid_prices_down[k], spot_prices[k], expected_prices_down[:,k], expected_price_variance_down) \
               + bid_volumes_up[k] * spot_prices[k] * balancing_market.activation_prob_up(bid_prices_up[k], spot_prices[k], expected_prices_up[:,k], expected_price_variance_up) 
